...
void _managing_events(appdata_s *ad, Evas_Object *obj, void *event_info)
{
    ...
    /**
     * Other example:
     * The event may be all-day event:
...
/*
void _managing_todos(appdata_s *ad, Evas_Object *obj, void *event_info)
{
    ...
    PRINT_MSG("Creating a Todo");
    error_code = calendar_record_create(_calendar_todo._uri, &todo);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_create: %d", error_code);
        PRINT_MSG("calendar_record_create: %d", error_code);
    }

    // Setting Todo Properties
    const char *summary = "Todo summary";
    PRINT_MSG("Summary: %s", summary);
    ...
    const char *description = "Todo description";
    PRINT_MSG("Description: %s", description);
    ...
    error_code =
        calendar_record_set_int(todo, _calendar_todo.todo_status, CALENDAR_TODO_STATUS_COMPLETED);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_int() failed: %d", error_code);
        PRINT_MSG("calendar_record_set_int() failed: %d", error_code);
    }
    ...
    // Inserting a Todo into the Database
    PRINT_MSG("Inserting a Todo");
    error_code = calendar_db_insert_record(todo, &id);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_db_insert_record() failed: %d", error_code);
        PRINT_MSG("calendar_db_insert_record() failed: %d", error_code);
    } else {
        PRINT_MSG("Todo inserted successfully");

        todo_ids = eina_list_append(todo_ids, (void *)id);
    }
    ...
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_filter_create filter() failed: %d", error_code);
        PRINT_MSG("calendar_filter_create filter() failed: %d", error_code);
    }

    const char *summary_pattern = "Todo summary";
    error_code =
        calendar_filter_add_str(filter, _calendar_todo.summary, CALENDAR_MATCH_STARTSWITH,
                                summary_pattern);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_filter_add_str() failed: %d", error_code);
        PRINT_MSG("calendar_filter_add_str() failed: %d", error_code);
    ...
    const char *description_pattern = "Todo description";
    error_code =
        calendar_filter_add_str(filter, _calendar_todo.description, CALENDAR_MATCH_STARTSWITH,
                                description_pattern);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_filter_add_str() failed: %d", error_code);
        PRINT_MSG("calendar_filter_add_str() failed: %d", error_code);
    ...
    error_code =
        calendar_filter_add_int(filter, _calendar_todo.todo_status, CALENDAR_MATCH_EQUAL,
                                CALENDAR_TODO_STATUS_COMPLETED);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_filter_add_int() failed: %d", error_code);
        PRINT_MSG("calendar_filter_add_int() failed: %d", error_code);
    }

    error_code = calendar_query_set_filter(query, filter);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_query_set_filter filter(): %d", error_code);
        PRINT_MSG("calendar_query_set_filter() failed: %d", error_code);
    }
    ...
    PRINT_MSG("Summary starts with: %s", summary_pattern);
    PRINT_MSG("Description starts with: %s", description_pattern);
    PRINT_MSG("Status: Completed");

    PRINT_MSG("Getting Todos using the query");
    ...
    while (calendar_list_get_current_record_p(cal_list, &record) == CALENDAR_ERROR_NONE) {
        ...
        // You can get, for example, summary:
        PRINT_MSG("Summary: %s", gl_todo_data->summary);
    ...
    calendar_list_destroy(cal_list, true);
    cal_list = NULL;

    PRINT_MSG("");

    // Updating a Todo
    PRINT_MSG("Updating a Todo");
    ...
    const char *new_summary = "Updated Todo summary";
    PRINT_MSG("New summary: %s", new_summary);
    error_code = calendar_record_set_str(record, _calendar_todo.summary, new_summary);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_str() failed: %d", error_code);
        PRINT_MSG("calendar_record_set_str() failed: %d", error_code);
    }

    const char *new_description = "Updated Todo description";
    PRINT_MSG("New description: %s", new_description);
    error_code = calendar_record_set_str(record, _calendar_todo.description, new_description);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_str() failed: %d", error_code);
        PRINT_MSG("calendar_record_set_str() failed: %d", error_code);
    }
    ...
    if (error_code != CALENDAR_ERROR_NONE) {
        ...
        PRINT_MSG("Todo updated successfully");
...
static bool _vcalendar_parse_cb(calendar_record_h record, void *user_data)
{
    ...
    int id = -1;
    int error_code = calendar_db_insert_record(record, &id);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_db_insert_record() failed: %d", error_code);
        PRINT_MSG("calendar_db_insert_record() failed: %d", error_code);
        return false;
    ...
    dlog_print(DLOG_DEBUG, LOG_TAG, "Inserted id: %d", id);
    PRINT_MSG("Inserted id: %d", id);

    return true;
}

void _managing_vcalendars(appdata_s *ad, Evas_Object *obj, void *event_info)
{
    calendar_record_h record = NULL;
    ...
    int error_code = calendar_record_create(_calendar_event._uri, &record);
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_record_create() failed: %d", error_code);
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_create() failed: %d", error_code);
    }

    error_code = calendar_record_set_str(record, _calendar_event.summary, "vCalendar event summary");
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_record_set_str() failed: %d", error_code);
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_str() failed: %d", error_code);
    }

    error_code = calendar_record_set_str(record, _calendar_event.description, "vCalendar event description");
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_record_set_str() failed: %d", error_code);
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_str() failed: %d", error_code);
    }

    error_code = calendar_record_set_str(record, _calendar_event.start_tzid, "Asia/Seoul");
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_record_set_str() failed: %d", error_code);
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_str() failed: %d", error_code);
    }
    ...
    starttime.type = CALENDAR_TIME_UTIME;
    starttime.time.utime = 1404036200;  // 2014/06/29 10:00:00 UTC
    error_code = calendar_record_set_caltime(record, _calendar_event.start_time, starttime);
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_record_set_caltime() failed: %d", error_code);
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_caltime() failed: %d", error_code);
    }
    ...
    endtime.type = CALENDAR_TIME_UTIME;
    endtime.time.utime = 1404036000 + 3600; // 2014/06/29 11:00:00 UTC
    error_code = calendar_record_set_caltime(record, _calendar_event.end_time, endtime);
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_record_set_caltime() failed: %d", error_code);
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_caltime() failed: %d", error_code);
    }

    error_code = calendar_record_set_int(record, _calendar_event.freq, CALENDAR_RECURRENCE_MONTHLY);
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_record_set_int() failed: %d", error_code);
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_int() failed: %d", error_code);
    }

    error_code = calendar_record_set_int(record, _calendar_event.interval, 1);
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_record_set_int() failed: %d", error_code);
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_int() failed: %d", error_code);
    }

    error_code = calendar_record_set_str(record, _calendar_event.bymonthday, "3,4,5");
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_record_set_str() failed: %d", error_code);
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_str() failed: %d", error_code);
    }

    error_code = calendar_record_set_int(record, _calendar_event.range_type, CALENDAR_RANGE_COUNT);
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_record_set_int() failed: %d", error_code);
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_int() failed: %d", error_code);
    }

    error_code = calendar_record_set_int(record, _calendar_event.count, 4);
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_record_set_int() failed: %d", error_code);
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_set_int() failed: %d", error_code);
    }
    ...
    ct.type = CALENDAR_TIME_UTIME;
    ct.time.utime = 1404036000 - 60;    // 60 sec before starttime (1404036000)
    error_code = calendar_record_add_child_record(record, _calendar_event.calendar_alarm, alarm);
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_add_child_record() failed: %d",
                   error_code);
        created = false;
    }
    ...
    if (error_code != CALENDAR_ERROR_NONE) {
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_record_add_child_record() failed: %d",
                   error_code);
        PRINT_MSG("calendar_record_add_child_record() failed: %d", error_code);
    }

    error_code = calendar_db_insert_record(record, &event_id);
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_db_insert_record() failed: %d", error_code);
    } else {
        event_ids = eina_list_append(event_ids, (void *)event_id);
    }
    ...
    calendar_list_h cal_list = NULL;
    error_code = calendar_list_create(&cal_list);
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_list_create() failed: %d", error_code);
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_list_create() failed: %d", error_code);
    }

    error_code = calendar_list_add(cal_list, record);
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_list_add() failed: %d", error_code);
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_list_add() failed: %d", error_code);
    }

    error_code = calendar_vcalendar_make_from_records(cal_list, &vcalendar_stream);
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_vcalendar_make_from_records() failed: %d", error_code);
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_vcalendar_make_from_records() failed: %d",
                   error_code);
    }

    PRINT_MSG("vCalendar stream:");
    PRINT_MSG("%s", vcalendar_stream);
    PRINT_MSG("");
    ...
    // Parsing a vCalendar
    char vcalendar_file_path[BUFLEN];
    char *path = app_get_resource_path();
    snprintf(vcalendar_file_path, BUFLEN, "%svcalendar.ics", path);
    free(path);

    error_code =
        calendar_vcalendar_parse_to_calendar_foreach(vcalendar_file_path, _vcalendar_parse_cb,
                NULL);
    if (error_code != CALENDAR_ERROR_NONE) {
        PRINT_MSG("calendar_vcalendar_parse_to_calendar_foreach() failed: %d", error_code);
        dlog_print(DLOG_ERROR, LOG_TAG, "calendar_vcalendar_parse_to_calendar_foreach() failed: %d",
                   error_code);
    }
...
void _contacts_page(appdata_s *ad)
{
    // Setting the window
	...
	// Create a naviframe
	    ...
	    create_buttons_in_main_window2(ad);
...
void _contacts_management(appdata_s *ad, Evas_Object *obj, void *event_info)
{
    ...
    // Following code is commented to have preview of changes
    //    contact_id = id;            // Acquire contact ID
    //    int unlinked_person_id = -1;
    //    error_code = contacts_person_unlink_contact(person_id, contact_id, &unlinked_person_id);
    //    if (error_code != CONTACTS_ERROR_NONE){
    //        PRINT_MSG("contacts_person_unlink_contact() failed: %d", error_code);
    //        dlog_print(DLOG_ERROR, LOG_TAG, "contacts_person_unlink_contact() failed: %d", error_code);
    //    }
...
void create_buttons_in_main_window(appdata_s *ad)
{
	 Evas_Object *display = _create_new_cd_display(ad, "Main", _pop_cb, ad->navi);
	 	_new_button(ad, display, "Bluetooth", _bluetooth_page);
...
void create_buttons_in_main_window2(appdata_s *ad)
{
	...
	Evas_Object *display = _create_new_cd_display(ad, "Contacts", _pop_cb,ad->navi1);
		contacts_init();
...
void create_buttons_in_main_window3(appdata_s *ad)
{
    ...
    //_new_button(ad, display, "Managing Todos", _managing_todos);
    _new_button(ad, display, "Managing vCalendars ", _managing_vcalendars);
